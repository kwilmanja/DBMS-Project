...
const StoryController = (app) => {
        ...
        pool.query('call create_prompt(?, ?, ?);',
            [name, description, username], (err, results) => {
                if (err) {
                    ...
                    const promptId = results[0][0].id;

                    const query = "insert into describe_prompt (prompt_id, genre) values " +
                        "(" + promptId + ", \'" + genres.join("\'), (" + promptId + ", \'") + "\');";
        ...
        pool.query('select * from genre;',
            [], (err, results) => {
                if (err) {
                    ...
                    res.status(500).json({ error: 'Failed to find genres' });
                    return;
                } else {
                    res.json(results);
                }
    ...
    app.get("/api/prompt/genres", getAllGenres);
